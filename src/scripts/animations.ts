...
if(!reduced){
  ...
  const observer=new IntersectionObserver(entries=>entries.forEach(e=>{if(e.isIntersecting){if(e.target.matches('.path-layout'))animate('.path-step',{opacity:[0,1],x:[45,0],duration:780,delay:stagger(140),ease:'out(3)'});else if(e.target.matches('.approach-panels'))animate('.approach-panels article',{opacity:[0,1],y:[50,0],duration:850,delay:stagger(130),ease:'out(3)'});else if(e.target.matches('.meeting-options'))animate('.meeting-options article',{opacity:[0,1],y:[36,0],duration:800,delay:stagger(120),ease:'out(3)'});else reveal(e.target);observer.unobserve(e.target)} }),{threshold:.16});
  document.querySelectorAll('.manifesto-title,.manifesto-copy,.path-heading,.path-layout,.approach-panels,.profile-mark,.profile-copy,.meeting-intro,.meeting-options,.contact>*').forEach(el=>observer.observe(el));
  const progress=document.querySelector<HTMLElement>('.path-progress'),section=document.querySelector<HTMLElement>('.path-section');
  if(progress&&section){const update=()=>{const r=section.getBoundingClientRect(),p=Math.max(0,Math.min(1,(innerHeight*.7-r.top)/(r.height+innerHeight*.4)));progress.style.height=`${p*100}%`};addEventListener('scroll',update,{passive:true});update()}
}
